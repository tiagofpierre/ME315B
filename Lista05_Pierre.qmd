...
```{sql connection=con}
SELECT PNAME, FEE, (FEE + 50.0) / 2 AS NOVA_TAXA FROM PRESERVE
```
...
```{sql connection=con}
SELECT AVG(0.02 * ACRES) AS MEDIA_NOVA FROM PRESERVE
WHERE STATE = 'AZ'
```
...
```{sql connection=con}
SELECT SUM(SALARY) AS SOMA_SALARIOS, AVG(SALARY) AS MEDIA_SALARIAL, MAX(SALARY) AS MAIOR_SALARIO, MIN(SALARY) AS MENOR_SALARIO FROM EMPLOYEE
```
